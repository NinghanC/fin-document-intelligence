  ...
  DocParserAgent              KnowledgeExtractAgent          ChromaDB/PGVector + Neo4j
          ...
          +------------------------------+------------------------------+
                              ...
                              QAAgent / GraphRAGPipeline
                                         |
                                         v
                              Cited answer + retrieval quality
```
...
The public prototype scores each retrieval branch against the query and fuses results with reciprocal rank fusion. This avoids hand-picked source weights while still letting vector, subgraph, path, and community-summary contexts contribute. Community summaries are precomputed and cached from graph components during ingestion/update rather than generated by an LLM on every query.
